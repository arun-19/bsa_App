...
WHERE A.HREMPLOYMASTID = B.HREMPLOYMASTID AND B.BAND = C.HRBANDMASTID 
AND B.DESIGNATION = E.GTDESIGNATIONMASTID AND F.GTDEPTDESGMASTID = B.DEPTNAME
AND D.GTCOMPMASTID = A.COMPCODE AND B.IDACTIVE = 'YES' AND B.DOJ <= TO_DATE(TO_DATE(:DOCDATE, 'DD/MM/YYYY'), 'DD/MM/YYYY')  AND LOWER(:ROL) <> 'group role' 
AND ( D.COMPCODE = :COMPCODE OR 'ALL' = :COMPCODE ) AND ( F.MNNAME1 = :DEPT OR 'ALL' = :DEPT )  AND ( C.BANDID = :PAYCAT OR :PAYCAT = 'ALL')
GROUP BY A.GENDER,C.BANDID,E.DESIGNATION,F.MNNAME1
UNION ALL
...
AND C.DOJ <= TO_DATE(TO_DATE(:DOCDATE, 'DD/MM/YYYY'), 'DD/MM/YYYY') AND ( G.COMPCODE = :COMPCODE OR 'ALL' = :COMPCODE ) AND ( I.MNNAME1 = :DEPT OR 'ALL' = :DEPT ) 
AND LOWER(:ROL) = 'group role' and D.EMPTYPE = 'Permanent' AND ( H.BANDID = :PAYCAT OR :PAYCAT = 'ALL')
AND B.ODATE = TO_DATE(TO_DATE(:DOCDATE, 'DD/MM/YYYY'), 'DD/MM/YYYY') 
UNION 
SELECT H.BANDID,D.GENDER,C.IDCARD,F.DESIGNATION DEPT,I.MNNAME1 DEPTNAME 
FROM HRONDUTY A,HRONDUTYDET B,HREMPLOYDETAILS C,HREMPLOYMAST D,GTDESIGNATIONMAST F,GTCOMPMAST G,HRBANDMAST H,GTDEPTDESGMAST I
WHERE A.HRONDUTYID = B.HRONDUTYID AND B.IDCARD = C.HREMPLOYDETAILSID AND D.HREMPLOYMASTID = C.HREMPLOYMASTID
AND C.DESIGNATION = F.GTDESIGNATIONMASTID AND G.GTCOMPMASTID = D.COMPCODE AND C.BAND = H.HRBANDMASTID AND I.GTDEPTDESGMASTID = C.DEPTNAME
AND C.DOJ <= TO_DATE(TO_DATE(:DOCDATE, 'DD/MM/YYYY'), 'DD/MM/YYYY') AND ( G.COMPCODE = :COMPCODE OR 'ALL' = :COMPCODE ) AND ( I.MNNAME1 = :DEPT OR 'ALL' = :DEPT ) AND LOWER(:ROL) <> 'group role' 
AND B.ODATE = TO_DATE(TO_DATE(:DOCDATE, 'DD/MM/YYYY'), 'DD/MM/YYYY')  AND ( H.BANDID = :PAYCAT OR :PAYCAT = 'ALL')
UNION
...
(
SELECT TO_TIMESTAMP( TO_DATE(TO_DATE(:DOCDATE, 'DD/MM/YYYY'), 'DD/MM/YYYY') ||' '||MIN(A.STST),'DD/MM/YY HH24:MI:SS') - NUMTODSINTERVAL( :ADNLT ,'HOUR')  STST 
...
ORDER BY A.BANDID,A.DEPTNAME
        ...
        const params = {
            ...
            ROL: { val: 'Admin Role' },
            ADNLT: { val: '0.5' },
            DDDATE: { val: '03/02/2025' },
            DOCDATE: { val: '01/02/2025' }
        };
...
(
SELECT MAX(ZA.ATTDATE) FROM "$ATT" ZA WHERE ZA.IDCARD = A.IDCARD AND ZA.ATTDATE <= :DOCDATE
) LASTPUNCHDT  
FROM (
...
WHERE A.HOSGTNENTRYID=B.HOSGTNENTRYID AND :DOCDATE BETWEEN B.GNDATE AND B.RETDATE AND B.IDCARDNO=C.IDCARD
))  STATUS,
(SELECT 'LEAVE' TYPE FROM HRLEAVEREGMAST A,HREMPLOYDETAILS B,HRBANDMAST C 
WHERE A.IDCARD = B.HREMPLOYMASTID AND C.HRBANDMASTID = B.PAYCAT AND A.LRDATE = :DOCDATE AND B.IDCARD=C.IDCARD  
...
WHERE  A.HREMPLOYMASTID=B.HREMPLOYMASTID AND  B.HREMPLOYMASTID=C.HREMPLOYMASTID AND A.ATTDATE=:DOCDATE
...
AND A.DOCDATE=:DOCDATE AND D.COMPCODE=:COMPCODE
AND B.OHDAY='I & II' 
) WHERE ORD=1))) A
